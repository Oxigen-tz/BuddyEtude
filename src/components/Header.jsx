...
const Header = () => {
  ...
  // FONCTION POUR LES LIENS DU MENU
  const NavLink = ({ to, label }) => {
      const isActive = location.pathname === to;
      return (
        <Link 
            to={to} 
            className={`px-3 py-2 rounded-lg font-bold text-sm transition flex items-center gap-2 ${
                isActive 
      ...
      <div className="container mx-auto flex justify-between items-center">
        ...
        {user ? (
          <div className="flex items-center gap-3">
            ...
            {/* --- NOUVEAU BOUTON MESSAGES --- */}
            <Link 
                to="/dashboard" 
                className="relative bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-full transition flex items-center gap-2 group border border-white/20"
                title="Accéder à mes discussions"
            >
                {/* Icône SVG propre */}
                ...
                {unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold min-w-[18px] h-[18px] flex items-center justify-center rounded-full border-2 border-blue-600 animate-bounce">
                        {unreadCount}
                    </span>
                )}
            </Link>
            ...
            <button 
              onClick={logout} 
              className="bg-blue-900/30 hover:bg-red-500 text-white w-9 h-9 rounded-full flex items-center justify-center transition ml-1"
              title="Déconnexion"
            >
